...
const getNikeJobBoard = async() => {
  const res = await request.get(process.env.NIKE_JOB_BOARD);
  const { meta, jobs: firstJobs } = res.body;

  let allJobs = firstJobs;

  for(let i = 1; i < meta.pagination.pages; i++) {
    const offset = i * 36;

    const res = await request.get(`${process.env.NIKE_JOB_BOARD}&offset=${offset}`);
    allJobs = [...allJobs, ...res.body.jobs];
  }

  allJobs.forEach(job => job.id = job.atsId);
